<script lang="ts">
  ...
  function createMap(element: HTMLDivElement) {
    ...
    $mapInstance.on("error", (e) =>
      dispatch("error", { ...e, map: $mapInstance })
    );

    $mapInstance.on("movestart", (ev) =>
      dispatch("movestart", { ...ev, map: $mapInstance })
    );
    ...
    $mapInstance.on("zoomstart", (ev) =>
      dispatch("zoomstart", { ...ev, map: $mapInstance })
    );
